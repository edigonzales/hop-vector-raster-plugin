 ...
 *
 * <p>The {@link FileChannel} is owned by the dataset writer; {@link #close()} therefore does not
 * close the channel.
 */
public final class DbfWriter implements AutoCloseable {
  ...
  private byte[] encodeField(DbfField field, Object value) throws ShapefileMappingException {
      ...
      if (field.type() == DbfFieldType.CHARACTER
        ...
        throw new ShapefileMappingException(
            "DBF field '"
                ...
                + "' encodes to "
                + encoded.length
                + " bytes but the field width is "
                + length
                + " (overflow policy is "
                + overflowPolicy.name().toLowerCase()
                + ")");
      }
    }
    ...
    boolean rightJustify =
        field.type() == DbfFieldType.NUMERIC || field.type() == DbfFieldType.FLOAT;
  ...
  private String formatDate(Object value) throws ShapefileMappingException {
    ...
    try {
      ...
      throw new ShapefileMappingException(
          "DBF date field: cannot interpret value '" + value + "' (expected yyyyMMdd or ISO date)",
          e);
    }
  }
