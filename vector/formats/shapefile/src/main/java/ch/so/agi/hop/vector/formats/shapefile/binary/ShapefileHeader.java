...
public record ShapefileHeader(
  ...
  public static ShapefileHeader read(FileChannel channel)
    ...
    while (raw.hasRemaining()) {
      ...
      if (n < 0) {
        throw new IOException(
            "Unexpected end of file reading SHP header: expected " + HEADER_SIZE + " bytes");
      }
    }
    buf.flip();
  ...
  public void validateMainFileHeader() throws ShapefileMappingException {
    if (fileCode != FILE_CODE_EXPECTED) {
      throw new ShapefileMappingException(
          "SHP file code is " + fileCode + ", expected " + FILE_CODE_EXPECTED);
    }
    if (version != VERSION_EXPECTED) {
      throw new ShapefileMappingException(
          "SHP version is " + version + ", expected " + VERSION_EXPECTED);
    }
  }
