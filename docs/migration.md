...
Stop Hop before installing manually. Remove both the previous plugin folder and an existing `vector-raster` folder, then extract the new ZIP into Hop home. Install `hop-geometry-type-plugin` separately as before for the shared Hop value-type registration; the Vector/Raster ZIP carries the exact Geometry runtime and JTS runtime needed by its direct vector/raster consumers. Do not copy additional JARs manually. The development installation script performs the old-folder cleanup automatically.
